...
TSHttpTxnSsnGet(TSHttpTxn txnp)
{
  ...
  return reinterpret_cast<TSHttpSsn>(sm->ua_transaction ? (TSHttpSsn)sm->ua_transaction->get_parent() : nullptr);
}
...
TSHttpTxnOutgoingAddrSet(TSHttpTxn txnp, const struct sockaddr *addr)
{
  ...
  sm->ua_transaction->set_outbound_port(ats_ip_port_host_order(addr));
  sm->ua_transaction->set_outbound_ip(IpAddr(addr));
  return TS_ERROR;
}
...
TSHttpTxnOutgoingTransparencySet(TSHttpTxn txnp, int flag)
{
  ...
  HttpSM *sm = reinterpret_cast<HttpSM *>(txnp);
  if (nullptr == sm || nullptr == sm->ua_transaction) {
    return TS_ERROR;
  }

  sm->ua_transaction->set_outbound_transparent(flag);
  return TS_SUCCESS;
}
...
TSHttpTxnClientPacketMarkSet(TSHttpTxn txnp, int mark)
{
  sdk_assert(sdk_sanity_check_txn(txnp) == TS_SUCCESS);
  HttpSM *sm = (HttpSM *)txnp;
  if (nullptr == sm->ua_transaction) {
    return TS_ERROR;
  }

  NetVConnection *vc = sm->ua_transaction->get_netvc();
  if (nullptr == vc) {
    return TS_ERROR;
  }
...
TSHttpTxnServerPacketMarkSet(TSHttpTxn txnp, int mark)
{
  ...
  if (nullptr != sm->ua_transaction) {
    HttpServerSession *ssn = sm->ua_transaction->get_server_session();
    if (nullptr != ssn) {
      NetVConnection *vc = ssn->get_netvc();
      if (vc != nullptr) {
        vc->options.packet_mark = (uint32_t)mark;
...
TSHttpTxnServerPush(TSHttpTxn txnp, const char *url, int url_len)
{
  ...
  HttpSM *sm          = reinterpret_cast<HttpSM *>(txnp);
  Http2Stream *stream = dynamic_cast<Http2Stream *>(sm->ua_transaction);
  if (stream) {
