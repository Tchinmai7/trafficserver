...
{
  ua_transaction->set_half_close_flag(true);
}
...
HttpSM::attach_client_session(ProxyClientTransaction *client_vc, IOBufferReader *buffer_reader)
{
  ...
  ua_transaction = client_vc;

  // Collect log & stats information
  client_tcp_reused         = !(ua_transaction->is_first_transaction());
  ...
  ink_release_assert(ua_transaction->get_half_close_flag() == false);
  mutex = client_vc->mutex;
  HTTP_INCREMENT_DYN_STAT(http_current_client_transactions_stat);
  if (ua_transaction->debug()) {
    debug_on = true;
  }
...
HttpSM::setup_client_read_request_header()
{
  ...
  ua_entry->read_vio = ua_transaction->do_io_read(this, INT64_MAX, ua_buffer_reader->mbuf);
...
{
  NetVConnection *netvc     = ua_transaction->get_netvc();
  SSLNetVConnection *ssl_vc = dynamic_cast<SSLNetVConnection *>(netvc);
  int host_len;
  if (ssl_vc && ssl_vc->GetSNIMapping()) {
    if (!t_state.hdr_info.client_request.url_get()->host_get(&host_len)) {
      ...
      if (hs != nullptr) {
        ...
        if (hs->port > 0) {
          ...
          t_state.hdr_info.client_request.url_get()->port_set(t_state.state_machine->ua_transaction->get_netvc()->get_local_port());
        }
      } else {
    ...
    ats_ip_ntop(t_state.state_machine->ua_transaction->get_netvc()->get_local_addr(), new_host, sizeof(new_host));
...
HttpSM::state_read_client_request_header(int event, void *data)
{
  ...
  if ((ua_buffer_reader->read_avail() > 0) && (client_request_hdr_bytes == 0)) {
    ...
    ua_transaction->set_inactivity_timeout(HRTIME_SECONDS(t_state.txn_conf->transaction_no_activity_timeout_in));
  }
  /////////////////////
  // tokenize header //
  /////////////////////
  ...
  case PARSE_RESULT_DONE:
    ...
    ua_transaction->set_session_active();
    ...
    if (t_state.hdr_info.client_request.version_get() == HTTPVersion(1, 1) &&
      ...
      if ((len == HTTP_LEN_100_CONTINUE) && (strncasecmp(expect, HTTP_VALUE_100_CONTINUE, HTTP_LEN_100_CONTINUE) == 0)) {
        ...
        ua_entry->write_vio = ua_transaction->do_io_write(this, nbytes, buf_start);
      }
    }
...
HttpSM::state_drain_client_request_body(int event, void *data)
{
  ...
  ink_assert(ua_entry->read_vio == (VIO *)data);
  ink_assert(ua_entry->vc == ua_transaction);
...
HttpSM::state_watch_for_client_abort(int event, void *data)
{
  ...
  case VC_EVENT_EOS: {
    // We got an early EOS.
    NetVConnection *netvc = ua_transaction->get_netvc();
    if (ua_transaction->allow_half_open()) {
      if (netvc) {
        netvc->do_io_shutdown(IO_SHUTDOWN_READ);
      }
      ua_entry->eos = true;
    } else {
      ua_transaction->do_io_close();
  ...
  case VC_EVENT_INACTIVITY_TIMEOUT: {
    if (tunnel.is_tunnel_active()) {
      ...
      HttpTunnelConsumer *c = tunnel.get_consumer(ua_transaction);
      if (c && c->alive) {
...
HttpSM::setup_push_read_response_header()
{
  ink_assert(server_session == nullptr);
  ink_assert(server_entry == nullptr);
  ink_assert(ua_transaction != nullptr);
    ...
    if (unlikely(t_state.did_upgrade_succeed)) {
      ...
      if (t_state.is_websocket) {
        ...
        if (ua_transaction) {
          ...
          ua_transaction->set_active_timeout(HRTIME_SECONDS(t_state.txn_conf->websocket_active_timeout));
          ua_transaction->set_inactivity_timeout(HRTIME_SECONDS(t_state.txn_conf->websocket_inactive_timeout));
        }
...
HttpSM::state_http_server_open(int event, void *data)
{
  ...
  case NET_EVENT_OPEN:
    ...
    /*UnixNetVConnection * vc = (UnixNetVConnection*)(ua_transaction->client_vc);
...
HttpSM::state_read_server_response_header(int event, void *data)
{
  ...
  if (server_response_hdr_bytes == 0) {
    ...
    if (ua_transaction && t_state.hdr_info.request_content_length) {
      ua_transaction->cancel_inactivity_timeout();
    }
  }
  /////////////////////
  // tokenize header //
  ...
  case PARSE_RESULT_DONE:
    ...
    /*
       if (ua_transaction && !t_state.hdr_info.request_content_length) {
       ua_transaction->get_netvc()->set_inactivity_timeout(HRTIME_SECONDS(
...
HttpSM::process_hostdb_info(HostDBInfo *r)
{
  ...
  if (use_client_addr) {
    NetVConnection *vc = t_state.state_machine->ua_transaction ? t_state.state_machine->ua_transaction->get_netvc() : nullptr;
    if (vc) {
...
HttpSM::state_hostdb_lookup(int event, void *data)
{
  ...
  case EVENT_SRV_LOOKUP: {
    ...
    opt.host_res_style = ua_transaction->get_host_res_style();
...
HttpSM::state_cache_open_write(int event, void *data)
{
  ...
  if (ua_transaction) {
    if ((pending_action = ua_transaction->adjust_thread(this, event, data))) {
...
HttpSM::tunnel_handler_post(int event, void *data)
{
  ...
  HttpTunnelProducer *p = ua_transaction != nullptr ? tunnel.get_producer(ua_transaction) : tunnel.get_producer(HT_HTTP_CLIENT);
  if (!p) {
  ...
  case VC_EVENT_INACTIVITY_TIMEOUT: // ua_transaction timeout during sending the HTTP 408 response
  case VC_EVENT_ACTIVE_TIMEOUT:     // ua_transaction timeout
    if (ua_entry->write_buffer) {
      free_MIOBuffer(ua_entry->write_buffer);
      ua_entry->write_buffer = nullptr;
...
HttpSM::tunnel_handler_push(int event, void *data)
{
  ...
  HttpTunnelProducer *ua = (ua_transaction) ? tunnel.get_producer(ua_transaction) : tunnel.get_producer(HT_HTTP_CLIENT);
...
HttpSM::tunnel_handler_server(int event, HttpTunnelProducer *p)
{
  ...
  if (close_connection) {
    ...
    */
    if (ua_transaction && ua_transaction->is_outbound_transparent() && t_state.http_config_param->use_client_source_port) {
      t_state.client_info.keep_alive = HTTP_NO_KEEPALIVE;
    }
  } else {
    ...
    // origin server
    if (t_state.txn_conf->attach_server_session_to_client == 1 && ua_transaction && t_state.client_info.keep_alive == HTTP_KEEPALIVE) {
      Debug("http", "attaching server session to the client");
      ua_transaction->attach_server_session(server_session);
    } else {
...
HttpSM::tunnel_handler_100_continue_ua(int event, HttpTunnelConsumer *c)
{
  ...
  ink_assert(c->vc == ua_transaction);
...
HttpSM::tunnel_handler_ua(int event, HttpTunnelConsumer *c)
{
  ...
  case VC_EVENT_WRITE_COMPLETE:
    ...
    if (t_state.client_info.keep_alive == HTTP_KEEPALIVE) {
      if (t_state.www_auth_content != HttpTransact::CACHE_AUTH_SERVE || ua_transaction->get_server_session()) {
  ...
  if (close_connection) {
    ...
    //   set the ua_transaction into half close mode
    ...
    if (is_eligible_post_request) {
      NetVConnection *vc = ua_transaction->get_netvc();
      if (vc) {
    ...
    ua_transaction->release(ua_buffer_reader);
    ua_buffer_reader = nullptr;
    // ua_transaction       = NULL;
  }
...
HttpSM::tunnel_handler_post_ua(int event, HttpTunnelProducer *p)
{
  ...
  case VC_EVENT_ACTIVE_TIMEOUT:
    if (client_response_hdr_bytes == 0) {
      ...
      SMDebug("http_tunnel", "send 408 response to client to vc %p, tunnel vc %p", ua_transaction->get_netvc(), p->vc);
      ...
      ua_transaction->set_active_timeout(HRTIME_SECONDS(t_state.txn_conf->transaction_active_timeout_in));
...
HttpSM::tunnel_handler_post_server(int event, HttpTunnelConsumer *c)
{
  ...
  case VC_EVENT_ACTIVE_TIMEOUT:
    ...
    ink_assert(ua_producer->vc == ua_transaction);
...
HttpSM::do_http_server_open(bool raw)
{
  ...
  // Make sure we are on the "right" thread
  if (ua_transaction) {
    if ((pending_action = ua_transaction->adjust_thread(this, EVENT_INTERVAL, nullptr))) {
  ...
  // to be based on ua_transaction != NULL instead of req_flavor value.
  ...
  if (raw == false && TS_SERVER_SESSION_SHARING_MATCH_NONE != t_state.txn_conf->server_session_sharing_match &&
    ...
    shared_result = httpSessionManager.acquire_session(this,                                 // state machine
                                                       ...
                                                       ua_transaction,                           // has ptr to bound ua sessions
                                                       this                                  // sm
                                                       );
  ...
  // session when we already have an attached server session.
           ...
           (ua_transaction != nullptr)) {
    HttpServerSession *existing_ss = ua_transaction->get_server_session();
    ...
    if (existing_ss) {
      ...
      if (ats_ip_addr_port_eq(&existing_ss->get_server_ip().sa, &t_state.current.server->dst_addr.sa)) {
        ua_transaction->attach_server_session(nullptr);
  ...
  // to get a new one.
  // ua_transaction is null when t_state.req_flavor == REQ_FLAVOR_SCHEDULED_UPDATE
  else if (ua_transaction != nullptr) {
    HttpServerSession *existing_ss = ua_transaction->get_server_session();
    if (existing_ss) {
      existing_ss->get_netvc()->set_inactivity_timeout(HRTIME_SECONDS(t_state.txn_conf->keep_alive_no_activity_timeout_out));
      existing_ss->release();
  ...
  if (ua_transaction) {
    opt.local_port = ua_transaction->get_outbound_port();

    const IpAddr &outbound_ip = AF_INET6 == ip_family ? ua_transaction->get_outbound_ip6() : ua_transaction->get_outbound_ip4();
    if (outbound_ip.isValid()) {
      opt.addr_binding = NetVCOptions::INTF_ADDR;
      opt.local_ip     = outbound_ip;
    } else if (ua_transaction->is_outbound_transparent()) {
      ...
      */
      ...
      NetVConnection *client_vc = ua_transaction->get_netvc();
...
HttpSM::release_server_session(bool serve_from_cache)
{
  ...
  if (TS_SERVER_SESSION_SHARING_MATCH_NONE != t_state.txn_conf->server_session_sharing_match && t_state.current.server != nullptr &&
    ...
    if (t_state.www_auth_content == HttpTransact::CACHE_AUTH_NONE || serve_from_cache == false) {
      ...
      ua_transaction->attach_server_session(server_session, false);
    }
  } else {
...
HttpSM::handle_post_failure()
{
  ...
  if (false == t_state.redirect_info.redirect_in_process) {
    ua_entry->read_vio = ua_transaction->do_io_read(this, 0, nullptr);
  }
  ua_entry->in_tunnel     = false;
  server_entry->in_tunnel = false;
...
HttpSM::do_setup_post_tunnel(HttpVC_t to_vc_type)
{
  ...
  if (p->handler_state != HTTP_SM_POST_UA_FAIL && ua_transaction->get_half_close_flag()) {
    p->vc->do_io_shutdown(IO_SHUTDOWN_READ);
  }
}
...
HttpSM::attach_server_session(HttpServerSession *s)
{
  ...
  } else { // If there is no ua_transaction no sense in continuing to attach the server session
    return;
  }
  ...
  UnixNetVConnection *client_vc = (UnixNetVConnection *)(ua_transaction->get_netvc());
...
HttpSM::setup_server_read_response_header()
{
  ...
  ink_assert(ua_transaction != nullptr || t_state.req_flavor == HttpTransact::REQ_FLAVOR_SCHEDULED_UPDATE ||
             t_state.req_flavor == HttpTransact::REQ_FLAVOR_REVPROXY);
...
HttpSM::setup_100_continue_transfer()
{
  ...
  ua_transaction->set_half_close_flag(false);
  ua_entry->in_tunnel = true;
  tunnel.tunnel_run(p);
}
...
HttpSM::setup_blind_tunnel(bool send_response_hdr, IOBufferReader *initial)
{
  ...
  // If we're half closed, we got a FIN from the client. Forward it on to the origin server
  // now that we have the tunnel operational.
  if (ua_transaction->get_half_close_flag()) {
...
HttpSM::kill_this()
{
  ...
  if (kill_this_async_done == true) {
    ...
    if (ua_transaction) {
      ua_transaction->transaction_done();
    }
...
HttpSM::update_stats()
{
  ...
  if (t_state.txn_conf->slow_log_threshold != 0 && ink_hrtime_from_msec(t_state.txn_conf->slow_log_threshold) < total_time) {
    ...
    if (ua_transaction != nullptr) {
      vc = ua_transaction->get_netvc();
      if (vc != nullptr) {
        fd = vc->get_socket();
      } else {
        fd = -1;
    ...
    Error("[%" PRId64 "] Slow Request: "
          ...
          "plugin_active: %.3f "
          "plugin_total: %.3f",
          sm_id, client_ip, t_state.client_info.src_addr.host_order_port(), ua_transaction ? ua_transaction->get_protocol_string() : "-1",
  ...
  case HttpTransact::SM_ACTION_DNS_LOOKUP: {
    ...
    if (t_state.api_server_addr_set) {
      ...
      break;
               ...
               t_state.dns_info.os_addr_style == HttpTransact::DNSLookupInfo::OS_Addr::OS_ADDR_TRY_DEFAULT &&
               ats_is_ip(addr = t_state.state_machine->ua_transaction->get_netvc()->get_local_addr())) {
      ...
      if (t_state.cop_test_page) {
        NetVConnection *vc = t_state.state_machine->ua_transaction->get_netvc();
        if (vc) {
...
HttpSM::is_private()
{
  ...
  if (server_session) {
    ...
    HttpServerSession *ss = ua_transaction->get_server_session();
    if (ss) {
...
HttpSM::populate_client_protocol(ts::string_view *result, int n) const
{
  ...
  if (n > 0) {
    ...
    if (!proto.empty()) {
      result[retval++] = proto;
      if (n > retval && ua_transaction) {
        retval += ua_transaction->populate_protocol(result + retval, n - retval);
      }
    }
  }
  return retval;
...
HttpSM::client_protocol_contains(ts::string_view tag_prefix) const
{
  ...
  if (!proto.empty()) {
    ...
    if (prefix.size() <= proto.size() && 0 == strncmp(proto.data(), prefix.data(), prefix.size())) {
      ...
      retval = ua_transaction->protocol_contains(prefix);
    }
  }
  return retval;
}
