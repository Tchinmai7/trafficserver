...
HttpTransact::EndRemapRequest(State *s)
{
  ...
  s->server_info.is_transparent = s->state_machine->ua_transaction ? s->state_machine->ua_transaction->is_outbound_transparent() : false;

done:
  // We now set the active-timeout again, since it might have been changed as part of the remap rules.
  if (s->state_machine->ua_transaction) {
    s->state_machine->ua_transaction->set_active_timeout(HRTIME_SECONDS(s->txn_conf->transaction_active_timeout_in));
  }
  ...
  case BAD_INCOMING_RESPONSE:
    ...
    if (is_request_retryable(s) && s->current.attempts < max_connect_retries) {
      ...
      if (DNSLookupInfo::OS_Addr::OS_ADDR_TRY_CLIENT == s->dns_info.os_addr_style) {
        ...
        s->state_machine->ua_transaction->set_host_res_style(ats_host_res_match(&s->current.server->dst_addr.sa));
        TRANSACT_RETURN(SM_ACTION_DNS_LOOKUP, OSDNSLookup);
...
HttpTransact::initialize_state_variables_from_request(State *s, HTTPHdr *obsolete_incoming_request)
{
  ...
  if (s->state_machine->ua_transaction) {
    vc = s->state_machine->ua_transaction->get_netvc();
  }
  ...
  if (!s->txn_conf->keep_alive_enabled_in || (s->state_machine->ua_transaction && s->state_machine->ua_transaction->ignore_keep_alive())) {
    s->client_info.keep_alive = HTTP_NO_KEEPALIVE;
...
HttpTransact::initialize_state_variables_from_response(State *s, HTTPHdr *incoming_response)
{
  ...
  } else if (s->state_machine->ua_transaction && s->state_machine->ua_transaction->is_outbound_transparent() &&
...
HttpTransact::process_quick_http_filter(State *s, int method)
{
  ...
  if (s->state_machine->ua_transaction) {
    const AclRecord *acl_record = s->state_machine->ua_transaction->get_acl_record();
...
HttpTransact::build_error_response(State *s, HTTPStatus status_code, const char *reason_phrase_or_null, const char *error_body_type)
{
  ...
  if ((s->state_machine->ua_transaction && s->state_machine->ua_transaction->is_outbound_transparent()) &&
